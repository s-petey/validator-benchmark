...
describe("Many objects bench, check if all objects pass the validation", () => {
  describe("arri", () => {
    test("base", () => {
      expect(users.map((user) => a.validate(arri.baseSchema, user)).every((result) => result === true)).toBe(true);
    });
    test("details", () => {
      expect(users.map((user) => a.validate(arri.detailsSchema, user)).every((result) => result === true)).toBe(true);
    });
  });

  describe("ajv", () => {
    test("base", () => {
      expect(users.map((user) => ajv.baseSchema(user)).every((result) => result === true)).toBe(true);
    });
    test("details", () => {
      expect(users.map((user) => ajv.detailsSchema(user)).every((result) => result === true)).toBe(true);
    });
  });

  describe("joi", () => {
    test("base", () => {
      expect(users.map((user) => joi.baseSchema.validate(user)).every((result) => !result.error)).toBe(true);
    });
    test("details", () => {
      expect(users.map((user) => joi.detailsSchema.validate(user)).every((result) => !result.error)).toBe(true);
    });
  });

  describe("myzod", () => {
    test("base", () => {
      expect(users.map((user) => myzod.baseSchema.try(user)).every((result) => !(result instanceof Error))).toBe(true);
    });
    test("details", () => {
      expect(users.map((user) => myzod.detailsSchema.try(user)).every((result) => !(result instanceof Error))).toBe(
        true,
      );
    });
  });
  ...
  describe("yup", () => {
    test("base", () => {
      expect(
        users.map((user) => yup.baseSchema.isValidSync(user, { strict: true })).every((result) => result === true),
      ).toBe(true);
    });
    test("details", () => {
      expect(
        users.map((user) => yup.detailsSchema.isValidSync(user, { strict: true })).every((result) => result === true),
      ).toBe(true);
    });
  });
  ...
  describe("zod", () => {
    test("base", () => {
      expect(users.map((user) => zod.baseSchema.safeParse(user)).every((result) => !result.error)).toBe(true);
    });
    test("details", () => {
      expect(users.map((user) => zod.detailsSchema.safeParse(user)).every((result) => !result.error)).toBe(true);
    });
  });

  describe("zod4", () => {
    test("base", () => {
      expect(users.map((user) => zod4.baseSchema.safeParse(user)).every((result) => !result.error)).toBe(true);
    });
    test("details", () => {
      expect(users.map((user) => zod4.detailsSchema.safeParse(user)).every((result) => !result.error)).toBe(true);
    });
  });

  describe("arktype", () => {
    test("base", () => {
      expect(users.map((user) => arktype.baseSchema(user) instanceof ArkErrors).every((result) => !result)).toBe(true);
    });
    test("details", () => {
      expect(users.map((user) => arktype.detailsSchema(user) instanceof ArkErrors).every((result) => !result)).toBe(
        true,
      );
    });
  });
  ...
  describe("effect", () => {
    test("base", () => {
      expect(users.map((user) => Schema.decodeSync(effect.baseSchema)(user)).every((result) => !!result)).toBe(true);
    });
    test("details", () => {
  ...
  describe("valibot", () => {
    test("base", () => {
      expect(users.map((user) => parse(valibot.baseSchema, user)).every((result) => !!result)).toBe(true);
    });
    test("details", () => {
      expect(users.map((user) => parse(valibot.detailsSchema, user)).every((result) => !!result)).toBe(true);
    });
  });

  describe("typebox", () => {
    test("base", () => {
      expect(users.map((user) => Value.Check(typebox.baseSchema, user)).every((result) => result === true)).toBe(true);
    });
    test("details", () => {
      expect(users.map((user) => Value.Check(typebox.detailsSchema, user)).every((result) => result === true)).toBe(
        true,
      );
    });
  });
